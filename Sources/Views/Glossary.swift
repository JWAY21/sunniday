...
enum Glossary {
    ...
    static let all: [GlossaryEntry] = [
        GlossaryEntry(id: "hormone", term: "Hormone",
                      definition: "A chemical messenger the body makes in one place and sends elsewhere to change what cells do. Hormones aren't fuel and aren't consumed like food; they carry instructions. Vitamin D qualifies on every count: your skin manufactures it, your liver and kidney switch it on, and it then travels through the blood to tissues all over the body and tells genes to turn up or down. Calling it a vitamin is really a historical accident. Vitamins are, by definition, things you have to eat because your body can't make them, and this one your body builds itself, given light."),
        GlossaryEntry(id: "uvb", term: "UVB",
                      definition: "The narrow slice of sunlight that makes vitamin D. Sunlight arrives as a spread of wavelengths: infrared at the long end, which you feel as heat; then visible light running red, orange, yellow, green, blue, violet; then ultraviolet beyond what the eye can see. Ultraviolet splits again. UVA is longer, penetrates deeper into the skin, and drives tanning and ageing. UVB is shorter and higher energy, and it's the one that counts here. Only a narrow band of it, around 295 to 300 nanometres, carries the exact energy needed to break open 7-DHC and start vitamin D. UVB is also stopped by glass, which is why a sunny window gives you plenty of warmth and light but essentially no vitamin D."),
        GlossaryEntry(id: "holick", term: "Holick",
                      definition: "Michael F. Holick, the American endocrinologist behind most of the science this app rests on. His group worked out the photochemistry through the late 1970s and 80s: that UVB opens 7-DHC into previtamin D3, that body heat rather than light completes the conversion, and that the whole process self-limits at photoequilibrium so sunlight cannot poison you. He also produced the rules of thumb still quoted everywhere, including that one whole-body minimal erythemal dose is worth roughly 10,000 to 25,000 IU. Those particular figures are widely used but not unchallenged: they were derived under fluorescent lamps rather than real sunlight, and later work argues they need correcting. This app uses his shape and his anchors, with that correction applied."),
        GlossaryEntry(id: "erythema", term: "Erythema (skin reddening)",
                      definition: "The medical word for reddening of the skin. In sun science it means the visible pinkness of sunburn, which appears some hours after exposure and peaks around a day later, caused by UV damaging DNA in skin cells and triggering inflammation. \"Erythemal\" describes anything weighted to that response, and the UV index itself is erythemally weighted: it was built to predict burning, not vitamin D. That distinction matters throughout this app, because the wavelengths that burn you and the wavelengths that make vitamin D are not quite the same, so burn risk is only ever a rough proxy for vitamin D gain."),
        GlossaryEntry(id: "uva", term: "UVA (the ageing wavelengths)",
                      definition: "The longer, lower-energy half of ultraviolet. UVA makes up roughly 95% of the UV that reaches the ground, penetrates deeper into the skin than UVB, and passes straight through cloud and window glass. It drives tanning and most of what we call photoageing: loss of collagen, leathery texture, pigment spots. What it does not do well is make vitamin D, and per unit of energy it is a far weaker cause of sunburn than UVB. So UVA is largely what you are still getting once the vitamin D has gone: late in the day, deep in winter, or through a car window."),
        GlossaryEntry(id: "melanin", term: "Melanin",
                      definition: "The pigment that gives skin, hair and eyes their colour, and the body's own sunscreen. Cells deep in the skin manufacture it and pass it to their neighbours, where it forms little caps over the cell nuclei and absorbs UV before it can damage DNA. More melanin means more protection, which is why darker skin burns far more slowly. The trade-off is that the very same pigment also absorbs the UVB that makes vitamin D, so the same sunshine takes longer to reach the same result. What melanin changes is the clock, not the ceiling: for the same sunburn-equivalent dose, synthesis is broadly comparable across skin tones."),
        GlossaryEntry(id: "infrared", term: "Red and infrared light",
                      definition: "The part of sunlight at and beyond red, which you feel as warmth rather than use to see. It makes no vitamin D and carries no burn risk. It gets attention because red and near-infrared light is absorbed by an enzyme in your mitochondria, and in the laboratory that can nudge cellular energy production. Two caveats are worth holding onto. Almost all the human research uses lamps or lasers at controlled doses rather than sunlight, and the clinical results so far are mixed and mostly low-certainty. And while low sun is proportionally richer in red and infrared, which is exactly why sunsets look the way they do, the absolute amount of every wavelength is lower too, so \"more infrared in the morning\" is true as a ratio and not as a quantity."),
        GlossaryEntry(id: "secosteroid", term: "Secosteroid",
                      definition: "A steroid is a molecule built from four carbon rings fused together, and cholesterol, testosterone and cortisol are all steroids. A secosteroid is one where a saw has been taken to one of those rings, breaking it open (\"seco\" is Latin for cut). In vitamin D, that one broken ring is everything: it lets the otherwise rigid molecule flex into an open shape, and it's that shape that slots into the receptor which switches genes on. Weld the ring shut again and it stops working as a hormone."),
        GlossaryEntry(id: "sterol", term: "Sterol",
                      definition: "A family of waxy, fat-like molecules all built on the same four-ring skeleton, found in the membranes of nearly every living cell. Cholesterol is the famous animal one; plants make their own (phytosterols). 7-DHC and vitamin D belong to the same family. Sterols keep cell membranes firm and fluid at the same time, and they're the raw material every steroid hormone is carved from, vitamin D included."),
        GlossaryEntry(id: "7-dhc", term: "7-DHC (7-dehydrocholesterol)",
                      definition: "Chemically this is cholesterol with one extra double bond, sitting at the very last stop on the assembly line before finished cholesterol. Your skin deliberately keeps a pool of it, because that extra double bond is exactly the bond UVB light can break. It is also called provitamin D3: raw material stationed in the skin, waiting for light."),
        ...
        GlossaryEntry(id: "hmg-coa-reductase", term: "HMG-CoA reductase",
                      definition: "The enzyme that sets the pace of the entire sterol assembly line, converting HMG-CoA into mevalonate. Because it is the slowest step, it is the one the cell polices most tightly: when cholesterol is plentiful the cell destroys the enzyme, and when it runs short it builds more. It is also the target of statin drugs, which block it to lower cholesterol. In theory that should reduce 7-DHC and therefore vitamin D too. In practice trials have never convincingly shown statins lowering vitamin D, and a few report the opposite, which remains unexplained."),
        GlossaryEntry(id: "squalene", term: "Squalene",
                      definition: "A 30-carbon hydrocarbon, and the first molecule on the sterol line that is nearly the right size for the job. Six small five-carbon units join head to tail to build it. Then comes one of the more remarkable steps in biochemistry: the whole floppy chain folds up on itself and snaps closed into the four fused rings that every sterol carries. Your skin also secretes squalene onto its surface in sebum, where it acts as a natural emollient and antioxidant, and it is abundant in olive oil and, notoriously, in shark liver."),
        GlossaryEntry(id: "lanosterol", term: "Lanosterol",
                      definition: "The first true sterol on the line, formed the moment squalene folds and closes into its four rings. It is still a long way from finished. Turning lanosterol into cholesterol takes roughly nineteen further enzyme steps, trimming away three carbons and shuffling bonds around the skeleton. 7-DHC, the molecule your skin holds in reserve for sunlight, appears right at the end of that sequence, one single step before cholesterol itself."),
        ...
        GlossaryEntry(id: "photoproducts", term: "Lumisterol & tachysterol",
                      definition: "The two molecules previtamin D3 is pushed into once sunlight passes the ceiling. Neither is vitamin D, but they are not a dead end either. Under continued light the reaction runs both ways, so as previtamin D3 is drawn off to become vitamin D3, lumisterol and tachysterol can convert back into it. In effect they work as a buffer, soaking up the excess and releasing it again as room appears. They were also written off as inert waste for decades, and that turned out to be wrong too: skin enzymes convert them into a family of active compounds with antioxidant and skin-protective roles of their own. Either way, they exist only because of light, which is one reason a supplement cannot fully stand in for sun."),
