...
enum Glossary {
    ...
    static let all: [GlossaryEntry] = [
        ...
        GlossaryEntry(id: "modelled-trend", term: "Modelled trend (body-store reserve)",
                      definition: "The yellow line on the history chart. The bars show what you made each day; this line models the reserve those days build up. Vitamin D's storage form (25(OH)D) has a roughly 2–3 week half-life, so intake accumulates and slowly clears — a good week lifts the line, a lazy one lets it fall. 100% marks the reserve a sustained daily-goal habit would hold; it saturates, so twice the sun doesn't give twice the reserve (the real response is curvilinear). Crucially it is a modelled trend, not a blood level: the app estimates skin synthesis in mcg, which isn't calibrated to what a lab measures in your blood. Watch its direction, not the exact number. Full detail is in \"How It Works\"."),
