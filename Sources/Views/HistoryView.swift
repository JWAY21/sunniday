...
struct HistoryView: View {
    ...
    enum Period: String, CaseIterable {
        case day = "D", week = "W", month = "M", threeMonth = "3M"
    ...
    private func loadData() {
        ...
        let halfLifeDays = 20
        let seedDays = halfLifeDays * 3          // 60-day warm-up
        let fetchDays = period.displayDays + seedDays   // always at least 90 days total for 3M
    ...
    //
    // This is equivalent to asking: "given that any vitamin D synthesised k
    // days ago has decayed to decay^k of its original level, what is my
    // effective current daily synthesis rate?"  A rising line means you are
    // outpacing decay; a falling line means you are not getting enough.
    static func halfLifeWMA(allBars: [Bar], halfLifeDays: Double) -> [MAPoint] {
        let decay = pow(0.5, 1.0 / halfLifeDays)   // ≈ 0.9659 for 20 days
        var points: [MAPoint] = []
        ...
        for i in allBars.indices {
            var weightedSum = 0.0
            var totalWeight  = 0.0
            // Look back at most 3 half-lives; beyond that the weight is < 12.5%
